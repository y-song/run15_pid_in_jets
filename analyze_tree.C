void analyze_tree()
{
    ...
    TH2F *h_trackpt_tracknspi = new TH2F("h_trackpt_tracknspi", ";track p_{T} [GeV];track nsigmapi", 16, 2, 10, 100, -5, 5);
    TH2F *h_trackm2_tracknspi = new TH2F("h_trackm2_tracknspi", ";track M^{2} [GeV^{2}];track nsigmapi", 50, 0, 2, 100, -5, 5);

    TFile *f = new TFile("result/0426.root");
    TTree *t = (TTree *)f->Get("jets");

    Double_t pt_j = 0.0, eta_j = 0.0, phi_j = 0.0, m_j = 0.0;
    Int_t event_j, mult_j = 0;
    Double_t pt[100], eta[100], phi[100], nspi[100], m2[100];
    Short_t q[100];
    ...
    for (int ijet = 0; ijet < njet; ijet++)
    {
        ...
        for (int itrack = 0; itrack < mult_j; itrack++)
        {
            ...
            //h_trackpt_tracknspi->Fill(pt[itrack], nspi[itrack]);
